  ...
  return (
        ...
        mb-5 cursor-pointer'>
            <div className='flex items-center justify-between gap-5' onClick={toggleAccordion}>
                <h4 className='text-[16px] leading-7 lg:text-[22px]
                lg:leading-8 text-slate-600'>{item.question}</h4>
                <div className= {`${
                     isOpen && "bg-blue-600 text-white border-none"
                     } w-7 h-7 lg:w-8 lg:h-8 border border-solid border-[#141521] 
            ...
            font-[400] text-slate-600'>{item.content}</p>
            </div>}
        </div>
  )
}
